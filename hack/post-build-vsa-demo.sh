...
# Find the pod
GET_SERVICE_POD='kubectl get pods --selector app=conforma-knative-service -n conforma-knative-service'
show-then-run "$GET_SERVICE_POD"
SERVICE_POD=$(eval "$GET_SERVICE_POD -o json | jq -r .items[0].metadata.name")

# Show the pod logs
pause-then-run "kubectl logs $SERVICE_POD -n conforma-knative-service"

#-----------------------------------------------------------------------------
h1 "Pod logs for the job created by VSA service"

# Find the pod
GET_JOB_POD='kubectl get pods --selector app.kubernetes.io/name=vsa-generator -n conforma-knative-service'
show-then-run "$GET_JOB_POD"
JOB_POD=$(eval "$GET_JOB_POD -o json | jq -r .items[0].metadata.name")

# Show the pod logs
pause-then-run "kubectl logs $JOB_POD -n conforma-knative-service"
